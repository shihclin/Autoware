...
{
	enum FasterRcnnDetectorClasses
	{
		BACKGROUND,
...
public:
	/**
	* @brief Faster-RCNN Constructor, creates the Caffe network,  loads the definition(.proto) and pretrained model(.caffemodel), also sets GPU mode and  Device ID
	*/
	FasterRcnnDetector(std::string& network_definition_file,
